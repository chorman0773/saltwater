...
use saltwater_core::lazy_static::lazy_static;
use saltwater_core::shared_str::RcStr;
...
use saltwater_core::error::CppError;
use saltwater_core::lex::{Keyword, LiteralToken};
use saltwater_core::*;
use crate::Files;
...
impl<'a> PreProcessor<'a> {
    ...
    fn directive(&mut self, kind: DirectiveKind, start: u32) -> Result<(), CompileError> {
        use saltwater_core::error::Warning as WarningDiagnostic;
        use DirectiveKind::*;
        match kind {
            If => {
        ...
        loop {
            match self.file_processor.next_non_whitespace() {
                ...
                Some(Ok(Locatable {
                    ...
                    self.error_handler
                        .warn(saltwater_core::error::Warning::IgnoredVariadic, location);
                }
                Some(Ok(Locatable {
                    data: Token::Id(id),
                    ..
    ...
    fn include(&mut self, start: u32) -> Result<(), Locatable<Error>> {
        use saltwater_core::lex::ComparisonToken;
...
mod tests {
    use super::*;
    use saltwater_core::lex::test::{cpp, cpp_no_newline};
...
h",
    ...
    fn eof_after_macro_call() {
        use saltwater_core::lex::test::cpp_no_newline;
