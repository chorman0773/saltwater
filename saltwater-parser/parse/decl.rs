...
impl<I: Lexer> Parser<I> {
    ...
    pub fn external_declaration(&mut self) -> SyntaxResult<Locatable<ExternalDeclaration>> {
        ...
        if self.peek_token() == Some(&Token::LeftBrace) {
            use saltwater_core::ast::{DeclaratorType, FunctionDefinition};
        ...
        let has_typedef = specifiers
            .iter()
            .any(|s| *s == DeclarationSpecifier::Unit(saltwater_core::ast::UnitSpecifier::Typedef));
    ...
    pub fn type_name(&mut self) -> SyntaxResult<Locatable<TypeName>> {
        use saltwater_core::ast::DeclaratorType;
    ...
    fn struct_specifier(
        ...
        use saltwater_core::ast::StructSpecifier;
